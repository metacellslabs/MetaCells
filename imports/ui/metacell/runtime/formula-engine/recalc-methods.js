...
recalc(sheetId, sourceCellId, condition, targetCellRef, stack) {
        ...
        var stateKey = sheetId + ":" + String(sourceCellId).toUpperCase() + "->" + targetCellId;
        if (!shouldRecalc) {
            console.info("[recalc] condition=false", { sheetId: sheetId, sourceCellId: sourceCellId, targetCellId: targetCellId });
        ...
        if (wasTriggered) {
            console.info("[recalc] condition=true already triggered", { sheetId: sheetId, sourceCellId: sourceCellId, targetCellId: targetCellId });
            return true;
        }

        console.info("[recalc] condition changed false->true, forcing refresh", { sheetId: sheetId, sourceCellId: sourceCellId, targetCellId: targetCellId });
        this.deferRecalc(sheetId, targetCellId);

        return true;
    },

updateCellFormula(sourceSheetId, sourceCellId, targetCellRef, newFormulaValue) {
        var target = this.normalizeUpdateTarget(sourceSheetId, sourceCellId, targetCellRef);
        if (!target) return false;
        var nextRaw = String(newFormulaValue == null ? "" : newFormulaValue);
        var prevRaw = this.storageService.getCellValue(target.sheetId, target.cellId);
...
triggerRecalc(sheetId, targetCellId, stack) {
        try {
            console.info("[recalc] triggerRecalc now", { sheetId: sheetId, targetCellId: targetCellId });
            var value = this.aiService.withManualTrigger(() =>
                this.evaluateCell(sheetId, targetCellId, stack || {}, { forceRefreshAI: true })
            );
            console.info("[recalc] triggerRecalc done", { sheetId: sheetId, targetCellId: targetCellId, value: value });
        } catch (e) {
...
deferRecalc(sheetId, targetCellId) {
        ...
        if (this.recalcQueued[queueKey]) {
            console.info("[recalc] deferRecalc skipped (already queued)", { sheetId: sheetId, targetCellId: targetCellId });
            return;
        }
        this.recalcQueued[queueKey] = true;
        console.info("[recalc] deferRecalc scheduled", { sheetId: sheetId, targetCellId: targetCellId });
        setTimeout(() => {
            console.info("[recalc] deferRecalc executing", { sheetId: sheetId, targetCellId: targetCellId });
...
normalizeUpdateTarget(sourceSheetId, sourceCellId, targetCellRef) {
        ...
        if (targetCellRef == null) return { sheetId: sourceSheetId, cellId: sourceCell };

        var token = String(targetCellRef).trim();
        if (!token || /^cell$/i.test(token)) return { sheetId: sourceSheetId, cellId: sourceCell };
        if (token.charAt(0) === "@") token = token.substring(1).trim();

        var sheetCell = /^(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+)$/.exec(token);
        if (sheetCell) {
        ...
        if (localCell) return { sheetId: sourceSheetId, cellId: localCell[1].toUpperCase() };
