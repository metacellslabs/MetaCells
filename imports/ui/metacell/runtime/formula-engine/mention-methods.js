...
getMentionValue(sheetId, cellId, stack, options) {
        ...
        if (raw.charAt(0) === "=") {
            ...
            if (shortcutPrompt) return this.readListShortcutResult(sheetId, targetCellId, stack, options);
        }
        if (this.isTableShortcutRaw(raw)) {
            return this.readTableShortcutResult(sheetId, targetCellId, stack, options);
        }
...
getMentionRawValue(sheetId, cellId) {
        return String(this.storageService.getCellValue(sheetId, String(cellId || "").toUpperCase()) || "");
    },

getNamedRefValue(ref, stack, options, rawMode) {
        if (!ref || !ref.sheetId) return "";
        if (ref.startCellId && ref.endCellId) {
            if (rawMode) return this.regionToRawCsv(ref.sheetId, String(ref.startCellId).toUpperCase(), String(ref.endCellId).toUpperCase());
            return this.regionToCsv(ref.sheetId, String(ref.startCellId).toUpperCase(), String(ref.endCellId).toUpperCase(), stack || {}, options);
        }
        if (!ref.cellId) return "";
        if (rawMode) return this.getMentionRawValue(ref.sheetId, String(ref.cellId).toUpperCase());
        return this.getMentionValue(ref.sheetId, String(ref.cellId).toUpperCase(), stack, options);
    },
...
findReportTabId(currentSheetId, reportToken) {
        ...
        if (!token) {
            if (currentSheetId && this.isReportSheetId(currentSheetId)) return currentSheetId;
        ...
        for (var n = 0; n < reportTabs.length; n++) {
            ...
            if (this.normalizeReportToken(byName.name) === normalizedToken) return byName.id;
...
renderReportAsViewText(reportSheetId, html, stack, options) {
        ...
        markdown = markdown.replace(/Input:(@[A-Za-z_][A-Za-z0-9_]*|(?:@)?(?:'[^']+'|[A-Za-z][A-Za-z0-9 _-]*)[!:][A-Za-z]+[0-9]+)/g, (_, token) => {
            var refToken = String(token || "").trim();
            if (!refToken) return "";
            var normalized = refToken.charAt(0) === "@" ? refToken.substring(1) : refToken;
            try {
                if (/^(?:'[^']+'|[A-Za-z][A-Za-z0-9 _-]*)[!:][A-Za-z]+[0-9]+$/.test(normalized)) {
                    var m = /^(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))[!:]([A-Za-z]+[0-9]+)$/.exec(normalized);
                    if (!m) return "";
                    var sheetName = m[1] || m[2] || "";
                    var sheetId = this.findSheetIdByName(sheetName);
                    if (!sheetId) return "";
                    return String(this.getMentionValue(sheetId, String(m[3]).toUpperCase(), stack, options) || "");
                }
                return String(this.getPlainMentionValue(reportSheetId, normalized, stack, options) || "");
            } catch (e) {
                return "";
            }
        });

        return this.expandMentionsInPromptText(reportSheetId, markdown, stack, options);
    },
...
htmlToMarkdown(html) {
        ...
        if (!source) return "";
        try {
            if (typeof window !== "undefined" && typeof window.TurndownService === "function") {
...
normalizeReportToken(text) {
        return String(text == null ? "" : text).toLowerCase().replace(/[\s_-]+/g, "");
    },
