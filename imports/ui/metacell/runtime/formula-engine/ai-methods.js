...
arePromptDependenciesResolved(sheetId, text, options) {
        ...
        for (var i = 0; i < dependencies.length; i++) {
            ...
            if (dependency.kind === "cell") {
                if (!this.isCellDependencyResolved(dependency.sheetId, dependency.cellId)) return false;
                continue;
            }
...
collectAIPromptDependencies(sheetId, text) {
        ...
        if (!source) return [];
        var pattern = /@@?(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+):([A-Za-z]+[0-9]+)|@@?([A-Za-z]+[0-9]+):([A-Za-z]+[0-9]+)|@@?(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+)|@@?([A-Za-z_][A-Za-z0-9_]*)/g;
        var results = [];
        var seen = {};
        var m;
        ...
        while ((m = pattern.exec(source))) {
            ...
            var key = dependency.kind === "region"
                ? "region:" + dependency.sheetId + ":" + dependency.startCellId + ":" + dependency.endCellId
...
wrapResolvedMentionsForAI(sheetId, text, stack, options) {
        var source = String(text == null ? "" : text);
        if (!source) return "";
        var pattern = /(_)?@(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+):([A-Za-z]+[0-9]+)|(_)?@([A-Za-z]+[0-9]+):([A-Za-z]+[0-9]+)|(_)?@(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+)|(_)?@([A-Za-z_][A-Za-z0-9_]*)/g;
        ...
        return source.replace(pattern, (_, rangeRawPrefix, qSheetRange, pSheetRange, rangeStart, rangeEnd, localRangeRawPrefix, localRangeStart, localRangeEnd, sheetRawPrefix, qSheetCell, pSheetCell, sheetCellId, plainRawPrefix, plainToken) => {
            try {
                ...
                if (rangeStart && rangeEnd) {
                    ...
                    resolved = this.regionToCsv(rangeSheetId, rangeStart.toUpperCase(), rangeEnd.toUpperCase(), stack || {}, options);
                } else if (localRangeStart && localRangeEnd) {
                    resolved = this.regionToCsv(sheetId, localRangeStart.toUpperCase(), localRangeEnd.toUpperCase(), stack || {}, options);
                } else if (sheetCellId) {
        ...
        return source.replace(pattern, (_, rangeRawPrefix, qSheetRange, pSheetRange, rangeStart, rangeEnd, localRangeRawPrefix, localRangeStart, localRangeEnd, sheetRawPrefix, qSheetCell, pSheetCell, sheetCellId, plainRawPrefix, plainToken) => {
            try {
                if (rangeStart && rangeEnd) {
                    ...
                    var rangeSheetId = this.findSheetIdByName(rangeSheetName);
                    if (!rangeSheetId) return "";
                    return this.regionToCsv(rangeSheetId, rangeStart.toUpperCase(), rangeEnd.toUpperCase(), stack || {}, options);
                }

                if (localRangeStart && localRangeEnd) {
                    return this.regionToCsv(sheetId, localRangeStart.toUpperCase(), localRangeEnd.toUpperCase(), stack || {}, options);
                }
                ...
                if (plainToken) {
                    var plainValue = this.getPlainMentionValue(sheetId, plainToken, stack, options, !!plainRawPrefix);
...
tryDirectMentionTableSpill(sheetId, cellId, rawFormula, stack, options) {
        ...
        var targetRaw = String(this.storageService.getCellValue(target.sheetId, target.cellId) || "");
        if (!this.isTableShortcutRaw(targetRaw)) return null;

        var matrix = this.readTableShortcutMatrix(target.sheetId, target.cellId, stack, options);
...
resolveDirectMentionFormulaRef(sheetId, rawFormula) {
        ...
        var sheetCellMatch = /^(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+)$/.exec(token);
        if (sheetCellMatch) {
            var sheetName = sheetCellMatch[1] || sheetCellMatch[2] || "";
            var refSheetId = this.findSheetIdByName(sheetName);
            if (!refSheetId) return null;
        ...
        var named = this.storageService.resolveNamedCell(token);
        if (named && named.sheetId && named.cellId) {
            return { sheetId: named.sheetId, cellId: String(named.cellId).toUpperCase() };
        }
        return null;
    },
...
listAI(sheetId, sourceCellId, text, count, forceRefresh, stack, options, fillStartFromIndex) {
        ...
        var startIndex = typeof fillStartFromIndex === "number" ? fillStartFromIndex : 1;

        return this.aiService.list(prompt, total, (options) => {
    ...
    readListShortcutResult(sheetId, sourceCellId, stack, options) {
        ...
        if (!source) return "";
        var generatedIds = this.storageService.listGeneratedCellsBySource(sheetId, sourceCellId) || [];
        if (generatedIds.length) {
            var generatedValues = generatedIds
                ...
                .map((item) => {
                    var value = this.evaluateCell(sheetId, item.cellId, stack || {}, options);
    ...
    readTableShortcutResult(sheetId, sourceCellId, stack, options) {
        var matrix = this.readTableShortcutMatrix(sheetId, sourceCellId, stack, options);
        if (!matrix.length) return "";
        var lines = [];
        for (var r = 0; r < matrix.length; r++) {
            lines.push(matrix[r].map((v) => this.escapeCsv(String(v == null ? "" : v))).join(","));
        }
        return lines.join("\n");
    },
    ...
    readTableShortcutMatrix(sheetId, sourceCellId, stack, options) {
        ...
        if (generatedIds.length) {
            ...
            if (generatedCells.length) {
                ...
                for (var g = 1; g < generatedCells.length; g++) {
                    ...
                    if (generatedCells[g].row > maxRowGenerated) maxRowGenerated = generatedCells[g].row;
                    if (generatedCells[g].col < minCol) minCol = generatedCells[g].col;
                    if (generatedCells[g].col > maxColGenerated) maxColGenerated = generatedCells[g].col;
                }
                ...
                for (var rowIndex = minRow; rowIndex <= maxRowGenerated; rowIndex++) {
                    ...
                    for (var colIndex = minCol; colIndex <= maxColGenerated; colIndex++) {
                        ...
                        var generatedValue = this.evaluateCell(sheetId, targetCellId, stack || {}, options);
                        var stringValue = String(generatedValue == null ? "" : generatedValue);
        ...
        for (var col = startCol; col <= maxCol; col++) {
            ...
            var firstRaw = String(this.storageService.getCellValue(sheetId, firstRowCellId) || "");
    ...
    spillMatrixToSheet(sheetId, sourceCellId, matrix) {
        var source = this.parseCellId(sourceCellId);
        if (!source) return;
        ...
        for (var r = 0; r < matrix.length; r++) {
            ...
            for (var c = 0; c < rowValues.length; c++) {
                var targetRow = source.row + r;
                var targetCol = source.col + c;
                if (targetRow < 1 || targetCol < 1) continue;
                var targetCellId = this.columnIndexToLabel(targetCol) + targetRow;
                if (targetCellId === String(sourceCellId).toUpperCase()) continue;
    ...
    getGridBounds() {
        ...
        if (this.storageService && typeof this.storageService.listAllCellIds === "function") {
            var refs = this.storageService.listAllCellIds();
            for (var s = 0; s < refs.length; s++) {
                var match = /^([A-Za-z]+)([0-9]+)$/.exec(String(refs[s] && refs[s].cellId || "").toUpperCase());
                if (!match) continue;
            ...
            try {
                if (sheetCell) {
                    ...
                    if (refSheetId) {
                        ...
                        value = this.getMentionValue(refSheetId, refCell, stack, options);
                        key = sheetName + "!" + refCell;
                    }
                } else if (plainToken) {
                    value = this.getPlainMentionValue(sheetId, plainToken, stack, options, plainRawPrefix === "_");
                    key = plainToken;
                }
            } catch (e) {}

            if (key) {
                contextLines.push("- " + key + ": " + String(value == null ? "" : value));
            }
