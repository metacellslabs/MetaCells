...
expandDependencyToCells(dependency) {
        ...
        if (dependency.kind !== "region" || !dependency.sheetId || !dependency.startCellId || !dependency.endCellId) {
            return [];
        }
...
collectFormulaReferenceDependencies(sheetId, formulaText) {
        ...
        source.replace(/(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+):([A-Za-z]+[0-9]+)/g, (_, quoted, plain, startCellId, endCellId) => {
        ...
        source.replace(/\b([A-Za-z]+[0-9]+):([A-Za-z]+[0-9]+)\b/g, (_, startCellId, endCellId) => {
            pushDependency({
                kind: "region",
                sheetId: sheetId,
                startCellId: String(startCellId).toUpperCase(),
        ...
        source.replace(/(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+)/g, (_, quoted, plain, cellId) => {
            var refSheetId = this.findSheetIdByName(quoted || plain || "");
            if (!refSheetId) return _;
            pushDependency({
                kind: "cell",
...
collectCellDependencies(sheetId, cellId) {
        ...
        if (raw.charAt(0) === "'") {
            return this.collectFormulaReferenceDependencies(sheetId, raw.substring(1));
        }

        if (raw.charAt(0) === ">") {
            return this.collectFormulaReferenceDependencies(sheetId, this.parseListShortcutPrompt(raw));
        }

        if (raw.charAt(0) === "#") {
            var spec = typeof this.parseTablePromptSpec === "function" ? this.parseTablePromptSpec(raw) : null;
            return this.collectFormulaReferenceDependencies(sheetId, spec && spec.prompt ? spec.prompt : raw.substring(1));
        }
...
buildEvaluationPlan(sheetId) {
        ...
        for (var j = 0; j < order.length; j++) {
            ...
            for (var d = 0; d < dependencies.length; d++) {
                ...
                if (!dependency || String(dependency.sheetId || "") !== targetSheetId) continue;
