...
function estimateRowHeight(app, rowIndex, row) {
  ...
  return Math.max(24, Number(app && app.grid ? app.grid.defaultRowHeight || 24 : 24));
}
...
function buildRowPlaceholder(app, rowIndex, sourceRow) {
  ...
  var cellCount =
    sourceRow && sourceRow.cells && sourceRow.cells.length
      ...
      : Math.max(1, Number(app.gridCols || (app.grid && app.grid.cols) || 0) + 1);
  ...
  for (var cellIndex = 0; cellIndex < cellCount; cellIndex++) {
    var sourceCell =
      sourceRow && sourceRow.cells && sourceRow.cells[cellIndex]
        ...
        : null;
    var td = sourceCell ? sourceCell.cloneNode(false) : document.createElement('td');
...
function getPinnedRowIndexes(app, options) {
  ...
  if (includeSelection) {
    var selectionRange =
      app && typeof app.getSelectionRange === 'function'
        ? app.getSelectionRange()
        : app.selectionRange;
    if (selectionRange) {
      for (var rowIndex = selectionRange.startRow; rowIndex <= selectionRange.endRow; rowIndex++) {
        rows[rowIndex] = true;
      }
    }
  }
...
function applyMountedRowRange(app, keepRowMap) {
  ...
  if (app.table && app.table.rows && app.table.rows.length) {
    for (var rowIndex = 1; rowIndex < app.table.rows.length; rowIndex++) {
      ...
      if (!row || row.dataset.rowMounted === 'true' || rowMap[rowIndex]) continue;
      row.dataset.rowMounted = 'false';
    }
  }
  app.mountedRowRange =
...
function pruneOffscreenCellContent(app, keepRowMap) {
  ...
  for (var i = 0; i < mountedInputs.length; i++) {
    ...
    var focusProxy = cell.querySelector('.cell-focus-proxy');
    ...
    if (focusProxy && focusProxy.parentNode === cell) {
      cell.removeChild(focusProxy);
    }
    if (actions && actions.parentNode === cell) {
      cell.removeChild(actions);
    }
    if (fillHandle && fillHandle.parentNode === cell) {
      cell.removeChild(fillHandle);
    }
...
export function getViewportRowRange(app) {
  ...
  var viewportBottom = viewportTop + Math.max(0, Number(app.tableWrap.clientHeight || 0));
...
export function renderViewportRows(app, options) {
  ...
  var keepRowMap = collectViewportRowIndexes(app, targets, options);
  ...
  for (var targetIndex = 0; targetIndex < targets.length; targetIndex++) {
    var targetInput = targets[targetIndex];
    var rowIndex = getInputRowIndex(app, targetInput);
    if (!rowIndex || seenRows[rowIndex]) continue;
