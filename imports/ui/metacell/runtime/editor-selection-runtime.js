export function getEditorSelectionRange(app, input) {
  var target =
    input || (app && typeof app.getActiveEditorInput === 'function'
...
export function syncOverlayInputSelection(app, sourceInput) {
  if (!app || !app.editorOverlayInput || !sourceInput) return;
  var range = getEditorSelectionRange(app, sourceInput);
  setEditorSelectionRange(app, range.start, range.end, app.editorOverlayInput);
}
