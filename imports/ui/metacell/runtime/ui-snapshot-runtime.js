export function collectAppUiStateSnapshot(app) {
  ...
  return {
    tabs: app.getWorkbookTabs(),
    visibleSheetId: app.getVisibleSheetId(),
    editingOwnerSheetId: app.getEditingOwnerSheetId(),
    ...
    selectionFillRange:
      typeof app.getSelectionFillRange === 'function'
        ? app.getSelectionFillRange()
        : app.fillRange,
    crossSheetPickContext:
      typeof app.getCrossSheetPickContext === 'function'
    ...
    reportUi: {
      ...
      mode: reportMode,
      isView: reportMode === 'view',
      editorVisible: isReportActive && reportMode !== 'view',
      liveVisible: isReportActive && reportMode === 'view',
      commandsDisabled: reportMode === 'view',
      canPublish: isReportActive,
      canExportPdf: isReportActive,
      activeSheetId:
        typeof app.getVisibleSheetId === 'function'
          ? String(app.getVisibleSheetId() || '')
          : String(app.activeSheetId || ''),
      editorHtml:
        app.reportEditor && typeof app.reportEditor.innerHTML === 'string'
          ? app.reportEditor.innerHTML
          : '',
      liveHtml:
        app.reportLive && typeof app.reportLive.innerHTML === 'string'
          ? app.reportLive.innerHTML
          : String(app.lastReportLiveHtml || ''),
      render: {
        hasEditorContent: !!(
          app.reportEditor &&
          String(app.reportEditor.innerHTML || '').trim()
        ),
        hasLiveContent: !!(
          app.reportLive &&
          String(app.reportLive.innerHTML || '').trim()
        ),
        lastRenderedHtmlLength: String(app.lastReportLiveHtml || '').length,
        liveHtmlMatchesCache:
          String(
            app.reportLive && typeof app.reportLive.innerHTML === 'string'
              ? app.reportLive.innerHTML
              : '',
      ...
      toolbar:
        typeof app.getReportToolbarUiState === 'function'
          ? app.getReportToolbarUiState()
          : null,
    },
  };
}
...
function collectSelectionUiState(app, activeCellId) {
  ...
  if (
    ...
    var startCell =
      getCellByCoords(selectionRange.startRow, selectionRange.startCol);
    var endCell =
      getCellByCoords(selectionRange.endRow, selectionRange.endCol);
  ...
  var computeHeaderRects = () => {
    ...
    dependencyCellIds.forEach((cellId) => {
      ...
      if (parsedDependency.col >= 1 && parsedDependency.col <= maxCol) {
        var dependencyColRect = toRelativeRect(
          getHeaderCell(parsedDependency.col),
        );
        if (dependencyColRect) {
          result.dependencyCols.push(dependencyColRect);
        }
      }
      if (parsedDependency.row >= 1 && parsedDependency.row <= maxRow) {
        ...
        if (dependencyRowRect) {
          result.dependencyRows.push(dependencyRowRect);
        }
      }
    });
    if (
      ...
      for (var row = selectionRange.startRow; row <= selectionRange.endRow; row++) {
        if (row >= 1 && row <= maxRow) {
          result.activeRows.push(
            toRelativeRect(
              getRowHeaderCell(row),
            ),
          );
        }
      }
      if (selectionRange.startCol === 1 && selectionRange.endCol === maxCol) {
        for (var selectedRow = selectionRange.startRow; selectedRow <= selectionRange.endRow; selectedRow++) {
          if (selectedRow >= 1 && selectedRow <= maxRow) {
            result.selectedRows.push(
              toRelativeRect(
                getRowHeaderCell(selectedRow),
              ),
            );
          }
        }
      ...
      if (selectionRange.startRow === 1 && selectionRange.endRow === maxRow) {
        for (var selectedCol = selectionRange.startCol; selectedCol <= selectionRange.endCol; selectedCol++) {
          if (selectedCol >= 1 && selectedCol <= maxCol) {
            result.selectedCols.push(
              toRelativeRect(
                getHeaderCell(selectedCol),
              ),
            );
          }
        }
      ...
      if (
        ...
        result.selectedCorner = toRelativeRect(
          getCellByCoords(0, 0),
        );
      }
    ...
    if (activeCellId) {
      ...
      if (parsedActive) {
        if (parsedActive.col >= 1 && parsedActive.col <= maxCol) {
          var activeColRect = toRelativeRect(
            getHeaderCell(parsedActive.col),
          );
          if (activeColRect) result.activeCols.push(activeColRect);
        }
        if (parsedActive.row >= 1 && parsedActive.row <= maxRow) {
          var activeRowRect = toRelativeRect(
            getRowHeaderCell(parsedActive.row),
          );
