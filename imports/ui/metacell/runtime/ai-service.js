...
import { AI_LIST_DELIMITER, buildListSystemPrompt, buildTableSystemPrompt } from "./ai-prompts.js";
...
export class AIService {
    constructor(storageService, onInvalidate, queueContext) {
        ...
        this.getQueueActiveSheetId = typeof context.getActiveSheetId === "function"
    ...
    ask(text, options) {
        ...
        var opts = options || {};
        var systemPrompt = String(opts.systemPrompt == null ? "" : opts.systemPrompt);
        var cacheKey = "AI_CACHE:" + systemPrompt + "\n---\n" + prompt;
        var cached = this.loadCache(cacheKey);
        var mode = this.getMode();
        var requestForced = !!opts.forceRefresh;
        var suppressionBlocked = (this.suppressRequests || this.editDraftLocked) && !requestForced;
        var shouldRequest = (mode === AI_MODE.auto || this.manualTrigger || requestForced) && !suppressionBlocked;
        var forceRefresh = requestForced || mode === AI_MODE.manual || this.forceRefresh;
    ...
    list(text, count, onResult, options) {
        ...
        var cacheKey = "AI_LIST_CACHE:" + total + ":" + systemPrompt + "\n---\n" + prompt;
    ...
    loadCache(cacheKey) {
        if (Object.prototype.hasOwnProperty.call(this.cache, cacheKey)) return this.cache[cacheKey];
    ...
    hasInFlightWork() {
        ...
        for (var q in this.queuedAutoRequests) {
            if (Object.prototype.hasOwnProperty.call(this.queuedAutoRequests, q)) return true;
        }
        return false;
    }
    ...
    requestAsk(prompt, cacheKey, forceRefresh, systemPrompt, userContent, queueMeta) {
        ...
        this.enrichPromptWithFetchedUrls(prompt)
            ...
            .then((content) => {
                var answer = String(content || "").trim() || "(empty response)";
    ...
    requestList(prompt, count, cacheKey, forceRefresh, onResult, systemPrompt, userContent, queueMeta) {
        ...
        this.enrichPromptWithFetchedUrls(prompt)
            ...
            .then((content) => {
                var options = this.parseListOptions(String(content || ""), count);
    ...
    htmlToMarkdown(html) {
        ...
        var TurndownCtor = typeof window !== "undefined" ? window.TurndownService : null;
        if (TurndownCtor) {
            try {
    ...
    fetchUrlAsMarkdown(url) {
        ...
        return Meteor.callAsync("ai.fetchUrlMarkdown", url)
            .then((markdown) => {
                markdown = this.htmlToMarkdown(markdown);
                var maxChars = 50000;
                var finalMarkdown = markdown.length > maxChars ? markdown.slice(0, maxChars) : markdown;
    ...
    enrichPromptWithFetchedUrls(prompt) {
        ...
        for (var i = 0; i < urls.length; i++) {
            ...
            basePrompt = basePrompt.replace(new RegExp("<\\s*" + escaped + "\\s*>|" + escaped, "g"), "");
        }
    ...
    parseListOptions(text, count) {
        ...
        var lines = source.indexOf(AI_LIST_DELIMITER) !== -1
        ...
        if (!options[0]) {
            var fallback = source.split(/[;,]/).map(function(part) { return part.trim(); }).filter(Boolean);
            if (fallback.length) {
                options = fallback;
            }
        }
    ...
    parseTableResponse(text, colsLimit, rowsLimit) {
        ...
        var tableLines = lines.filter(function(line) { return /\|/.test(line); });
        ...
        if (!rows.length) {
            rows = source.map(function(line) {
                return String(line || "").split(/\t|,/).map(function(cell) { return cell.trim(); }).filter(function(cell) { return cell !== ""; });
            }).filter(function(r) { return r.length > 0; });
        }

        if (colsLimit) {
            rows = rows.map(function(r) { return r.slice(0, colsLimit); });
        }
        if (rowsLimit) {
