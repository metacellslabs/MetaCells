...
function decodeMessageSource(sourceBuffer) {
  const raw = Buffer.isBuffer(sourceBuffer) ? sourceBuffer.toString("utf8") : String(sourceBuffer || "");
  if (!raw) return "";

  const separatorMatch = /\r?\n\r?\n/.exec(raw);
  const body = separatorMatch ? raw.slice(separatorMatch.index + separatorMatch[0].length) : raw;
  ...
  const text = /<html[\s>]/i.test(normalized) ? htmlToText(normalized) : normalized.replace(/\s+/g, " ").trim();
...
export async function testImapConnection(settings) {
  ...
  try {
    try {
      ...
      const message = formatNestedError(error) || "Failed to connect to email channel";
...
export async function pollImapMessages(settings, channel) {
  ...
  try {
    ...
    try {
      ...
      uids = Array.isArray(searchResult) ? searchResult.map((value) => Number(value) || 0).filter(Boolean) : [];
    ...
    for (let i = 0; i < uids.length; i += 1) {
      ...
      const payload = {
        ...
        subject: String(message.envelope && message.envelope.subject || "").trim(),
        ...
        date: message.internalDate instanceof Date ? message.internalDate.toISOString() : "",
