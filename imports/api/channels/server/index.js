...
import { normalizeChannelLabel, CHANNEL_POLL_INTERVAL_MS } from "../mentioning.js";
import { getActiveChannelPayloadMap } from "../runtime-state.js";
import { testImapConnection, sendImapMessage, handleImapEvent, pollImapMessages } from "./handlers/imap.js";
...
function normalizeChannelSettings(connector, currentSettings) {
  ...
  for (let i = 0; i < fields.length; i += 1) {
    ...
    next[key] = Object.prototype.hasOwnProperty.call(source, key) ? source[key] : defaultValue;
  }
  return next;
}
...
function getChannelHandler(connectorId) {
  ...
  throw new Meteor.Error("channel-connector-not-supported", `Unsupported channel connector: ${connectorId}`);
}
...
async function saveChannelRuntimeState(channelId, updates) {
  ...
  await AppSettings.updateAsync(
    { _id: DEFAULT_SETTINGS_ID, "communicationChannels.id": channelId },
    {
      $set: {
        ...Object.fromEntries(
          Object.keys(source).map((key) => [`communicationChannels.$.${key}`, source[key]]),
        ),
        updatedAt: new Date(),
      },
    },
...
async function triggerChannelMentionRecompute(channelLabel) {
  ...
  if (!label) return;
  const { recomputeSheetsMentioningChannel } = await import("../../sheets/index.js");
...
async function applyChannelEvent(channel, payload, nextUid) {
  ...
  const normalizedMessage = handled && handled.message && typeof handled.message === "object"
    ? {
        ...handled.message,
        event: String(handled.event || payload && payload.event || "message.new"),
...
async function pollSingleChannel(channel) {
  ...
  if (!channelId) {
    ...
    return { channelId: "", label: "", skipped: true, reason: "missing-id", events: 0 };
  }
  ...
  try {
    ...
    const lastSeenUid = Number(result && result.lastSeenUid) || Number(channel.lastSeenUid) || 0;
    ...
    const message = String((error && (error.reason || error.message)) || error || "Channel poll failed").trim();
...
async function pollEnabledChannels() {
  ...
  logChannelRuntime("poll.scan.start", {
    totalConfigured: Array.isArray(current && current.communicationChannels) ? current.communicationChannels.length : 0,
  ...
  const summary = {
    ...
    events: results.reduce((sum, item) => sum + (Number(item && item.events) || 0), 0),
    results,
  };
  ...
  Meteor.startup(() => {
    Meteor.setTimeout(() => {
      pollEnabledChannels().catch((error) => {
        logChannelRuntime("poll.startup.failed", { message: error && error.message ? error.message : String(error) });
      });
    }, 5000);
    Meteor.setInterval(() => {
      pollEnabledChannels().catch((error) => {
        logChannelRuntime("poll.interval.failed", { message: error && error.message ? error.message : String(error) });
      });
...
if (Meteor.isServer) {
  ...
  Meteor.methods({
    async "settings.upsertCommunicationChannel"(channel) {
      ...
      const connector = getRegisteredChannelConnectorById(channel.connectorId);
      if (!connector) {
        throw new Meteor.Error("channel-connector-not-found", "Channel connector not found");
      }
      ...
      const nextChannels = Array.isArray(current && current.communicationChannels)
        ? [...current.communicationChannels]
        : [];
      const existingChannel = nextChannels.find((item) => item && item.id === channel.id) || null;
      ...
      const normalizedChannel = {
        ...
        status: existingChannel && existingChannel.status ? existingChannel.status : "saved",
        settings: normalizeChannelSettings(connector, channel.settings),
        lastTestMessage: String(existingChannel && existingChannel.lastTestMessage || ""),
        lastTestAt: existingChannel && existingChannel.lastTestAt ? existingChannel.lastTestAt : null,
        lastSeenUid: Number(existingChannel && existingChannel.lastSeenUid) || 0,
        lastEvent: existingChannel && existingChannel.lastEvent && typeof existingChannel.lastEvent === "object"
          ? { ...existingChannel.lastEvent }
          : null,
        lastEventAt: existingChannel && existingChannel.lastEventAt ? existingChannel.lastEventAt : null,
        lastPolledAt: existingChannel && existingChannel.lastPolledAt ? existingChannel.lastPolledAt : null,
        watchError: String(existingChannel && existingChannel.watchError || ""),
        createdAt: existingChannel && existingChannel.createdAt ? existingChannel.createdAt : new Date(),
        updatedAt: new Date(),
      };

      const index = nextChannels.findIndex((item) => item && item.id === normalizedChannel.id);
      if (index === -1) nextChannels.push(normalizedChannel);
      else nextChannels[index] = { ...nextChannels[index], ...normalizedChannel };
    ...
    async "settings.testCommunicationChannel"(channelId) {
      ...
      await ensureDefaultSettings();
      const current = await AppSettings.findOneAsync(DEFAULT_SETTINGS_ID);
      const channels = Array.isArray(current && current.communicationChannels) ? current.communicationChannels : [];
      const channel = channels.find((item) => item && item.id === channelId);
      if (!channel) {
        throw new Meteor.Error("channel-not-found", "Communication channel not found");
      }
      ...
      const handler = getChannelHandler(connector.id);
      try {
        const result = await handler.testConnection(normalizeChannelSettings(connector, channel.settings));

        await AppSettings.updateAsync(
          { _id: DEFAULT_SETTINGS_ID, "communicationChannels.id": channelId },
          {
            $set: {
              "communicationChannels.$.status": result && result.ok ? "connected" : "error",
              "communicationChannels.$.lastTestAt": new Date(),
              "communicationChannels.$.lastTestMessage": String(result && result.message || ""),
              updatedAt: new Date(),
            },
          },
        );
    ...
    async "channels.send"(channelId, payload) {
      check(channelId, String);
      check(payload, Match.Where((value) => !!value && typeof value === "object" && !Array.isArray(value)));
