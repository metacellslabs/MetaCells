export function defineChannelConnector(definition) {
  ...
  return {
    ...
    sendParams: Array.isArray(source.sendParams)
      ? source.sendParams.map((item) => String(item || "").trim()).filter(Boolean)
      : [],
    mentioningFormulas: Array.isArray(source.mentioningFormulas)
      ? source.mentioningFormulas.map((item) => String(item || "").trim()).filter(Boolean)
      : [],
...
export function validateChannelConnectorDefinition(definition, origin) {
  ...
  if (!source) {
    throw new Error(`Channel connector module ${location} must export a default connector definition object`);
  }
  if (!String(source.id || "").trim()) {
    throw new Error(`Channel connector module ${location} is missing connector id`);
  }
  if (!String(source.type || "").trim()) {
    throw new Error(`Channel connector module ${location} is missing connector type`);
  }
  if (!String(source.name || "").trim()) {
    throw new Error(`Channel connector module ${location} is missing connector name`);
  }
